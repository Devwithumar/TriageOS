...
Scheduling is fail-closed by default. Set `SCHEDULING_PROVIDER=mock` only for deterministic local demonstrations. `SCHEDULING_PROVIDER=google_calendar` enables the configured Google Calendar adapter, which requires a calendar ID plus either an access token or refresh-token OAuth credentials; missing credentials fail closed without making a request.
